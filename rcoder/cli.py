...
def cmd_install(args):
    """install命令 - 完整安装（支持直连和中转模式）"""
    print_header("Rcoder 完整安装")
    
    # 询问使用模式
    print("请选择连接模式：")
    print("  1. 直连模式 - 直接连接目标服务器")
    print("  2. 中转模式 - 通过中转机连接目标服务器")
    mode_choice = input("请选择 (1/2，默认1): ").strip() or "1"
    
    use_proxy = mode_choice == "2"
    
    if use_proxy:
        print()
        print("【中转模式】")
        print("请配置中转机信息：")
        proxy_host = input(f"中转机IP (默认: ): ").strip()
        proxy_port = input(f"中转机端口 (默认: 8099): ").strip() or "8099"
        proxy_port = int(proxy_port)
        proxy_password = getpass.getpass(f"中转机密码 (默认: ): ")
        
        print()
        print("请配置目标机信息：")
        target_host = input(f"目标机IP (默认: 192.168.1.8): ").strip() or "192.168.1.8"
        target_port = input(f"目标机端口 (默认: 8099): ").strip() or "8099"
        target_port = int(target_port)
        target_password = getpass.getpass(f"目标机密码 (默认: ): ") or ""
        
        print()
        print("配置信息：")
        print(f"  中转机: {proxy_host}:{proxy_port}")
        print(f"  目标机: {target_host}:{target_port}")
        print()
    else:
        print()
        print("【直连模式】")
...
def send_command(cmd):
    try:
        print(f"[DEBUG] 中转模式: {{PROXY_HOST}}:{{PROXY_PORT}} -> {{TARGET_HOST}}:{{TARGET_PORT}}", file=sys.stderr)
        return send_via_proxy(cmd)
    except Exception as e:
...
def send_via_proxy(cmd):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    context = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
    context.load_verify_locations(CERT_FILE)
    context.check_hostname = False
    tls_sock = context.wrap_socket(sock, server_hostname=PROXY_HOST)
    tls_sock.connect((PROXY_HOST, PROXY_PORT))
    print("[DEBUG] TLS连接成功（中转机）", file=sys.stderr)
    req = json.dumps({{
        "command": cmd,
    ...
    print(f"[DEBUG] 发送命令到中转机: {{cmd[:50]}}...", file=sys.stderr)
    resp = tls_sock.recv(8192)
    print("[DEBUG] 收到中转机响应", file=sys.stderr)
    tls_sock.close()
...
def main():
    print(f"[Rcoder MCP客户端] 已启动（中转模式）", file=sys.stderr)
    while True:
        try:
            ...
            if method == "initialize":
                response["result"] = {{
                    ...
                    "capabilities": {{"tools": {{}}}},
                    "serverInfo": {{"name": "rcoder", "version": "1.1.0"}}
                }}
            ...
            error_resp = {{"jsonrpc": "2.0", "id": None, "error": {{"code": -32700, "message": f"JSON解析错误: {{str(e)}}"}}}}
            print(json.dumps(error_resp), flush=True)
        except Exception as e:
            import traceback
            traceback.print_exc(file=sys.stderr)
            error_resp = {{"jsonrpc": "2.0", "id": None, "error": {{"code": -32603, "message": f"{{type(e).__name__}}: {{str(e)}}"}}}}
            print(json.dumps(error_resp), flush=True)
...
def send_command(cmd):
    try:
        print(f"[DEBUG] 直连模式: {{REMOTE_HOST}}:{{REMOTE_PORT}}", file=sys.stderr)
        return send_direct(cmd)
    except Exception as e:
        print(f"[ERROR] {{type(e).__name__}}: {{str(e)}}", file=sys.stderr)
        import traceback
        traceback.print_exc(file=sys.stderr)
...
def send_direct(cmd):
    ...
    print("[DEBUG] TLS连接成功（直连）", file=sys.stderr)
...
def main():
    print(f"[Rcoder MCP客户端] 已启动（直连模式）", file=sys.stderr)
    while True:
        try:
            line = sys.stdin.readline()
            if not line:
